...
// Glifo Aperture · Áurea — A construído em proporção 1:1.618,
// ápice afilado, contra-forma verticalizada, barra-acento fina.
// `variant` controla o contexto visual: "inline" (glifo puro, usado em
// sidebars/topbars onde o wordmark fica ao lado) ou "badge" (dentro de
// um quadrado arredondado colorido — bom para avatares/app icons).
const BrandMark = ({ size = 30, variant = "inline", accent = true }) => {
  const glyph = (
    <svg width={size} height={size} viewBox="0 0 96 96" fill="none" style={{ flexShrink: 0 }}>
      <path d="M48 6 L86 82 L10 82 Z" fill="currentColor" />
      <path d="M48 32 L70 76 L26 76 Z" fill="var(--paper)" />
      {accent && <rect x="34" y="60" width="28" height="3" fill="var(--accent)" />}
    </svg>
  );
  if (variant === "badge") {
    return (
      <div style={{
        ...
        boxShadow: "inset 0 -2px 4px rgba(0,0,0,.18), 0 1px 0 rgba(255,255,255,.15)",
      }}>
        <svg width={size*0.62} height={size*0.62} viewBox="0 0 96 96" fill="none">
          <path d="M48 6 L86 82 L10 82 Z" fill="currentColor" />
          <path d="M48 32 L70 76 L26 76 Z" fill="var(--accent-deep)" opacity="0.35" />
          <rect x="34" y="60" width="28" height="3" fill="currentColor" />
        </svg>
      </div>
    );
  }
  // inline: glifo em ink, acento quente na barra
  return <span style={{ color: "var(--ink)", display: "inline-flex" }}>{glyph}</span>;
};
