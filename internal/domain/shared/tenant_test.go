...
func TestTenantStatusConstants(t *testing.T) {
	if TenantStatusActive != "active" {
		t.Errorf("TenantStatusActive should be 'active'")
	}
	if TenantStatusSuspended != "suspended" {
		t.Errorf("TenantStatusSuspended should be 'suspended'")
	}
}

func TestTenant_SuspendActivateCycle(t *testing.T) {
	tenant := NewTenant("id", "path")
	...
	tenant.Suspend()
	if tenant.IsActive() {
		t.Error("tenant should not be active after suspend")
	}
